...
var
  ...
  SQLOp: string;
  KeyName, JsonValue, SubParam: string; // Variables inline movidas aquí
...
begin
  ...
  try
    try
      ...
      Q.ExecSQL; // SQLDB no acepta parámetros
      {$ELSE}
      Q.ExecSQL('CREATE EXTENSION IF NOT EXISTS vector'); // FireDAC acepta SQL como parámetro
      {$ENDIF}
    except
    end;
