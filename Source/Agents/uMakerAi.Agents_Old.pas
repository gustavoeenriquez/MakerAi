...
{$RTTI INHERIT}
  ...
  TAgentPrintRef = reference to procedure(Sender: TObject; Value: String); //Tipo para ser llamado por procedimientos anónimos
  ...
  private
    ...
    FOnPrintRef: TAgentPrintRef;  //OnPrint para el método anonimo, sería conveniente hacer esto para todos los métodos.
  ...
  public
    ...
    // compatiblidad con métodos anónimos
    procedure SetOnPrintEvent(const APrintProc: TAgentPrintRef);  //Permite la asignación de un evento anónimo para el print
    // --- Runtime Builder API ---
...
begin
  if Assigned(Value) then  //Garantiza que el evento tiene prioridad sobre el evento anonimo
    FOnPrintRef := nil;
  FOnPrint := Value;
end;
...
begin
  // Guarda el procedimiento anónimo
  FOnPrintRef := APrintProc;

  // Asigna nuestro método "trampolín" al evento principal.
  // Si el procedimiento anónimo es nil, desconecta el evento.
  if Assigned(FOnPrintRef) then
