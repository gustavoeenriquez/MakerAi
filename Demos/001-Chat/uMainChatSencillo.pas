...
begin
  SetModelo('Gemini', 'gpt-4o-mini-tts');
end;
...
begin
  ShowMessage('Podría con whisper instalado en local');
end;
...
begin
  SetModelo('OpenAiResponse', 'gpt-4o-audio-preview');
  ChConAnexos.IsChecked := True;
end;
...
begin
  ...
  End
  Else
  Begin // La mayoría de las funcionalidades no incluyen ni tool ni asincrono
  ...
  // Pero si la petición genera algún archivo de medios lo mejor es obtenerlo del evento OnReceiveDataEnd
  ...
  // si no es asincrono se puede obtener el LastMessage y procesaro aquí
  // Ej:
