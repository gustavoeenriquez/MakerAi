program demo_groq_async;
{$mode objfpc}{$H+}

// Demo ASINCRONO/STREAMING con TAiGroqChat
// Groq expone un endpoint OpenAI SSE-compatible en https://api.groq.com/openai/v1/
//
...
begin
  ...
  if Sender is TAiGroqChat then
  begin
    Chat := TAiGroqChat(Sender);
...
begin
  ...
  WriteLn('Verifica que GROQ_API_KEY este configurado y sea valido.');
  Done := True;
end;
...
begin
  WriteLn('=== MakerAI FPC — Demo Groq ASYNC/STREAMING ===');
  WriteLn;
  ...
  try
    ...
    WriteLn;
    WriteLn('>>> Pregunta: Explica que es la IA generativa en 2 oraciones');
    WriteLn('<<< Respuesta (streaming):');
    WriteLn;

    Chat.AddMessageAndRun('Explica que es la IA generativa en 2 oraciones', 'user');
  ...
  WriteLn;
  WriteLn('Demo Groq async finalizado.');
end.
