program demo_openai_async;
{$mode objfpc}{$H+}

// Demo ASINCRONO/STREAMING con TAiOpenChat (Responses API)
// OpenAI Responses API usa SSE con eventos JSON tipados:
//   response.created, response.output_text.delta, response.completed, etc.
// El driver acumula el texto y llama OnReceiveData por cada delta.
//
...
begin
  ...
  if Sender is TAiOpenChat then
  begin
    Chat := TAiOpenChat(Sender);
...
begin
  ...
  WriteLn('Verifica que OPENAI_API_KEY este configurado y sea valido.');
  Done := True;
end;
...
begin
  WriteLn('=== MakerAI FPC — Demo OpenAI ASYNC/STREAMING ===');
  WriteLn;
  ...
  try
    ...
    WriteLn;
    WriteLn('>>> Pregunta: Explica brevemente que es OpenAI y la Responses API');
    WriteLn('<<< Respuesta (streaming):');
    WriteLn;

    Chat.AddMessageAndRun('Explica brevemente que es OpenAI y la Responses API', 'user');
  ...
  WriteLn;
  WriteLn('Demo OpenAI async finalizado.');
end.
