program demo_deepseek_async;
{$mode objfpc}{$H+}

// Demo ASINCRONO/STREAMING con TAiDeepSeekChat
// DeepSeek usa SSE estandar OpenAI-compatible.
// deepseek-reasoner adicionalmente devuelve "reasoning_content" en el delta
// que se acumula como ThinkingContent y se dispara via OnReceiveThinking.
//
...
procedure TDemoEvents.OnThinking(const Sender: TObject; aMsg: TAiChatMessage;
    aResponse: TJSONObject; aRole, aText: string);
begin
  // Recibe fragmentos de reasoning_content (solo deepseek-reasoner)
...
begin
  ...
  if Sender is TAiDeepSeekChat then
  begin
    Chat := TAiDeepSeekChat(Sender);
...
begin
  ...
  WriteLn('Verifica que DEEPSEEK_API_KEY este configurado y sea valido.');
  Done := True;
end;
...
begin
  WriteLn('=== MakerAI FPC — Demo DeepSeek ASYNC/STREAMING ===');
  WriteLn;
  ...
  try
    ...
    WriteLn;
    WriteLn('>>> Pregunta: Explica brevemente que es DeepSeek AI');
    WriteLn('<<< Respuesta (streaming):');
    WriteLn;

    Chat.AddMessageAndRun('Explica brevemente que es DeepSeek AI', 'user');
  ...
  WriteLn;
  WriteLn('Demo DeepSeek async finalizado.');
end.
